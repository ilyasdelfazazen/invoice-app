...
export class InvoiceStepperComponent implements OnInit, OnDestroy {
  ...
  private buildSteps(): void {
    this.steps = [
      this.translate.instant('invoices.stepClient'),
      this.translate.instant('invoices.stepLines'),
      this.translate.instant('invoices.stepSummary'),
    ];
  }
  ...
  getClientName(id: string): string {
    return this.clients.find(c => c._id === id)?.company || '—';
  }
  ...
  async submit(): Promise<void> {
    ...
    this.invoiceService.create(payload).subscribe({
      ...
      error: async () => {
        this.saving = false;
        const t = await this.toast.create({ message: 'Erreur lors de la création', duration: 4000, color: 'danger' });
        t.present();
      }
    });
  }
